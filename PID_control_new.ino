...
const uint8_t KP = 1; // Proportional gain term
const uint8_t KI = 1; // Integral gain term
const uint8_t KD = 1; // Derivative gain term
...
int calcPID(error_in,distanceToObstacle,distance,iteration_time) // add int_in if start condition is not zero
{
	...
	// Calculate the error
	error = distance - distanceToObstacle; // PID sensor distance - target distance
